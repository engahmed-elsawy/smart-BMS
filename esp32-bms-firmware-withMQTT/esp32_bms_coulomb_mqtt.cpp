...
/*
  ESP32 BMS Firmware — v2 — Coulomb Counting + Web Dashboard + MQTT
  ==================================================================
  ...
  Cell voltage signal chain:
    Cell differential → LM358 unity buffer → 1M/1M divider → ADC
    cellScale = 2.0
  ...
  MQTT topics published  (prefix = MQTT_TOPIC_PREFIX, default "bms"):
    bms/status         full JSON snapshot
    bms/soc            SOC % (Coulomb)
    bms/pack_voltage   pack V
    bms/pack_current   pack A
    bms/cell/1…6       individual cell voltages
    bms/charged_ah     cumulative Ah in
    bms/discharged_ah  cumulative Ah out

  MQTT command topics (subscribed):
    bms/cmd/balance_auto   ON | OFF
    bms/cmd/balance_all    OFF
    bms/cmd/balance_set    "N:S"  e.g. "3:1" or "3:0"
*/
...
const char*    MQTT_TOPIC_PREFIX  = "bms";

const uint32_t MQTT_PUBLISH_INTERVAL_MS = 5000;  // publish every 5 s
bool mqttEnabled = true;   // set false to run without MQTT
// ─────────────────────────────────────────────────────────────────────
...
static const int PIN_MUX_OUT = 34;
...
// =====================================================================
float packCapacityAh = 10.0f;   // ← set your real pack capacity in Ah

float chargeEfficiency    = 0.98f;
float dischargeEfficiency = 1.00f;

float    ccResyncCurrentThreshA = 0.20f;
uint32_t ccResyncRestDurationMs = 60000UL;
...
struct BmsData {
  ...
  float    avgCellVoltage, minCellVoltage, maxCellVoltage;
  float    socPercent, socVoltage;
  float    chargedAh, dischargedAh;
  bool     balanceState[6];
...
uint16_t readMuxRaw(uint8_t ch) {
  setMuxChannel(ch); delay(MUX_SETTLE_MS);
  uint32_t sum=0;
  for(uint8_t i=0;i<ADC_SAMPLES_PER_READ;i++){sum+=analogRead(PIN_MUX_OUT);delayMicroseconds(200);}
  return (uint16_t)(sum/ADC_SAMPLES_PER_READ);
}

float rawToVoltage(uint16_t raw){ return ((float)raw*ADC_VREF)/(float)ADC_MAX; }
...
void allBalanceOff(){ for(uint8_t i=0;i<6;i++) setBalance(i,false); }

float getMaxCellVoltage(){
  float v=bms.cellVoltage[0];
  for(uint8_t i=1;i<6;i++) if(bms.cellVoltage[i]>v) v=bms.cellVoltage[i];
  return v;
}
...
void updateCoulombCounting() {
  ...
  if(!ccInitialized){
    ...
    if(useMinCellForSocSafety) vIn=min(bms.avgCellVoltage,bms.minCellVoltage+0.03f);
    ccSocPercent=interpolateSocFromCellVoltage(vIn);
    bms.socPercent=ccSocPercent;
    ccLastSampleMs=ccRestStartMs=now;
    ccInitialized=true; return;
  }
  float dtH=(float)(now-ccLastSampleMs)/3600000.0f;
  ccLastSampleMs=now;
  float I=bms.packCurrent;
  if(I>0.0f){ float dAh=I*dtH*chargeEfficiency; bms.chargedAh+=dAh; ccSocPercent+=(dAh/packCapacityAh)*100.0f; }
  else if(I<0.0f){ float dAh=fabsf(I)*dtH/dischargeEfficiency; bms.dischargedAh+=dAh; ccSocPercent-=(dAh/packCapacityAh)*100.0f; }
  ccSocPercent=clampf(ccSocPercent,0.0f,100.0f);
  bool resting=(fabsf(I)<ccResyncCurrentThreshA);
  if(resting){
    if(!ccInRest){ccInRest=true;ccRestStartMs=now;}
    if((now-ccRestStartMs)>=ccResyncRestDurationMs){
      float vIn=bms.avgCellVoltage;
      if(useMinCellForSocSafety) vIn=min(bms.avgCellVoltage,bms.minCellVoltage+0.03f);
      ccSocPercent=ccSocPercent*0.90f+interpolateSocFromCellVoltage(vIn)*0.10f;
    }
  } else { ccInRest=false; }
  bms.socPercent=ccSocPercent;
  float vIn=bms.avgCellVoltage;
...
void updateAutoBalancing(){
  ...
  float maxC=getMaxCellVoltage(), minC=getMinCellVoltage();
  if((maxC-minC)<balanceStopDelta){allBalanceOff();return;}
  for(uint8_t i=0;i<6;i++){
    bool en=(bms.cellVoltage[i]>=balanceStartVoltage)&&((bms.cellVoltage[i]-minC)>=balanceStopDelta);
    setBalance(i,en);
  }
}
...
void sampleAllChannels(){
  for(uint8_t ch=0;ch<8;ch++){bms.raw[ch]=readMuxRaw(ch);bms.adcVoltage[ch]=rawToVoltage(bms.raw[ch]);}
  for(uint8_t i=0;i<6;i++) bms.cellVoltage[i]=bms.adcVoltage[i]*cellScale[i];
  ...
  updateAutoBalancing();
  bms.sampleCounter++;
  histBuf[histHead]={millis(),bms.packVoltage,bms.packCurrent,bms.socPercent,
...
String jsonStatus(){
  ...
  s+="\"cell_voltages\":[";
  for(uint8_t i=0;i<6;i++){s+=String(bms.cellVoltage[i],3);if(i<5)s+=",";}
  s+="],\"balance\":[";
  for(uint8_t i=0;i<6;i++){s+=bms.balanceState[i]?"true":"false";if(i<5)s+=",";}
  s+="],\"adc_raw\":[";
  for(uint8_t i=0;i<8;i++){s+=String(bms.raw[i]);if(i<7)s+=",";}
  s+="]}";
  return s;
}
...
String jsonHistory(){
  ...
  String s="{\"t\":[";
  for(uint16_t k=0;k<histCount;k++){uint16_t idx=(start+k)%HIST_SIZE;s+=String(histBuf[idx].t);if(k<histCount-1)s+=",";}
  s+="],\"packV\":[";
  for(uint16_t k=0;k<histCount;k++){uint16_t idx=(start+k)%HIST_SIZE;s+=String(histBuf[idx].packV,2);if(k<histCount-1)s+=",";}
  s+="],\"packI\":[";
  for(uint16_t k=0;k<histCount;k++){uint16_t idx=(start+k)%HIST_SIZE;s+=String(histBuf[idx].packI,2);if(k<histCount-1)s+=",";}
  s+="],\"soc\":[";
  for(uint16_t k=0;k<histCount;k++){uint16_t idx=(start+k)%HIST_SIZE;s+=String(histBuf[idx].soc,1);if(k<histCount-1)s+=",";}
  s+="],\"cells\":[";
  for(uint8_t c=0;c<6;c++){
    s+="[";
    for(uint16_t k=0;k<histCount;k++){uint16_t idx=(start+k)%HIST_SIZE;s+=String(histBuf[idx].cell[c],3);if(k<histCount-1)s+=",";}
    s+="]";if(c<5)s+=",";
  }
  s+="]}";
  return s;
...
const MAX_PTS=120;let autoBalOn=false;
function mkChart(id,label,color,yMin,yMax){return new Chart(document.getElementById(id),{type:'line',data:{labels:[],datasets:[{label,data:[],borderColor:color,borderWidth:1.5,pointRadius:0,fill:false,tension:0.3}]},options:{responsive:true,maintainAspectRatio:false,animation:false,scales:{x:{display:false},y:{min:yMin??undefined,max:yMax??undefined,ticks:{color:'#94a3b8',font:{size:10}},grid:{color:'rgba(255,255,255,.05)'}}},plugins:{legend:{display:false}}}});}
function mkCellChart(id){return new Chart(document.getElementById(id),{type:'line',data:{labels:[],datasets:COLORS.map((c,i)=>({label:'C'+(i+1),data:[],borderColor:c,borderWidth:1.2,pointRadius:0,fill:false,tension:0.3}))},options:{responsive:true,maintainAspectRatio:false,animation:false,scales:{x:{display:false},y:{ticks:{color:'#94a3b8',font:{size:10}},grid:{color:'rgba(255,255,255,.05)'}}},plugins:{legend:{labels:{color:'#94a3b8',font:{size:10},boxWidth:8}}}}});}
...
function push(chart,label,...vals){chart.data.labels.push(label);if(chart.data.labels.length>MAX_PTS)chart.data.labels.shift();vals.forEach((v,i)=>{chart.data.datasets[i].data.push(v);if(chart.data.datasets[i].data.length>MAX_PTS)chart.data.datasets[i].data.shift();});chart.update('none');}
...
async function tick(){
  try{
    const d=await(await fetch('/api/status')).json();const t=new Date().toLocaleTimeString();
    ...
    const bar=document.getElementById('socbar');bar.style.width=s.toFixed(1)+'%';bar.style.background=socColor(s);
...
void handleBalanceManual(){
  if(!server.hasArg("cell")||!server.hasArg("state")){server.send(400,"text/plain","?cell=1..6&state=0|1");return;}
  int cell=server.arg("cell").toInt(),state=server.arg("state").toInt();
  if(cell<1||cell>6){server.send(400,"text/plain","cell 1..6");return;}
  autoBalanceEnabled=false;setBalance(cell-1,state!=0);server.send(200,"text/plain","OK");
}

// =====================================================================
// MQTT
// =====================================================================
String mqttTopic(const char* sub){ return String(MQTT_TOPIC_PREFIX)+"/"+sub; }

void mqttPublishFloat(const char* sub,float val,int dec=3){
  if(!mqttClient.connected())return;
  mqttClient.publish(mqttTopic(sub).c_str(),String(val,dec).c_str(),true);
}

void mqttPublishAll(){
  if(!mqttClient.connected())return;
  mqttClient.publish(mqttTopic("status").c_str(),jsonStatus().c_str(),false);
  ...
  for(uint8_t i=0;i<6;i++){
    ...
    mqttClient.publish(mqttTopic(sub.c_str()).c_str(),String(bms.cellVoltage[i],3).c_str(),true);
  }
}
...
void mqttCallback(char* topic,byte* payload,unsigned int length){
  ...
  String tp(topic);
  if(tp==mqttTopic("cmd/balance_auto")){
    autoBalanceEnabled=(msg=="ON");
    if(!autoBalanceEnabled) allBalanceOff();
    Serial.printf("[MQTT] Auto balance %s\n",msg.c_str());
  } else if(tp==mqttTopic("cmd/balance_all")&&msg=="OFF"){
    autoBalanceEnabled=false; allBalanceOff();
    Serial.println("[MQTT] All balance OFF");
  } else if(tp==mqttTopic("cmd/balance_set")){
    int sep=msg.indexOf(':');
    if(sep>0){
      int cell=msg.substring(0,sep).toInt(), state=msg.substring(sep+1).toInt();
      if(cell>=1&&cell<=6){ autoBalanceEnabled=false; setBalance(cell-1,state!=0); }
      Serial.printf("[MQTT] Cell %d → %s\n",cell,state?"ON":"OFF");
    }
  }
}
...
void setup(){
  ...
  Serial.println("\nESP32 BMS v2 — Web Dashboard + MQTT — starting…");
  setupPins();setupAdc();allBalanceOff();
  setupWiFi();setupServer();setupMqtt();
  sampleAllChannels();
...
void loop(){
  static unsigned long lastSample=0,lastPrint=0,lastMqtt=0,lastRecon=0;
  ...
  if(mqttEnabled&&!mqttClient.connected()&&now-lastRecon>=10000){lastRecon=now;mqttReconnect();}
  if(mqttEnabled&&mqttClient.connected()){
    if(now-lastMqtt>=MQTT_PUBLISH_INTERVAL_MS){lastMqtt=now;mqttPublishAll();}
    mqttClient.loop();
  }
  server.handleClient();
}
